...
class WebGLFrame {
  ...
  setup() {
    ...
    for(let i = 0; i <= VERTEX_COUNT; i++) {
      let x = Math.sin(Math.PI * 2.0 * (i / VERTEX_COUNT));
      let z = Math.cos(Math.PI * 2.0 * (i / VERTEX_COUNT));
      ...
      for(let j = 0; j <= VERTEX_COUNT; j++) {
        ...
        let color = ((y + 1) / 2) * 360;
        let cColor = hsv2rgb(color, 1, 1);


        this.position.push(x * r * RADIUS, y * RADIUS, z * r * RADIUS);
        this.color.push(cColor[0], cColor[1], cColor[2], 1.0);
